...
def train(
    ...
    return best_val_loss, best_mean_recon, separation_metric
...
def train_k_fold(
    ...
    for train_index, val_index in kf.split(train_dataset, train_strata):
        ...
        val_loss, recon, separation_metric = train(
            config,
            model,
            train_loader,
            val_loader,
        ...
        print("separation_metric:", separation_metric)
        total_separation += separation_metric

        fold += 1

    return total_loss / n_splits, total_recon / n_splits, total_separation / n_splits


def main(config):
    average_val_loss, average_recon, average_separation = train_k_fold(config)

    return average_val_loss, average_recon, average_separation
...
if __name__ == "__main__":
    ...
    for batch_size, learning_rate, latent_dim, hidden_dim in tqdm(
        ...
        average_val_loss, average_recon, average_separation = main(config)

        results.append((config, average_val_loss, average_recon, average_separation))

    results_df = pd.DataFrame(
        results,
        columns=["config", "average_val_loss", "average_recon", "average_separation"],
    )

    results_df.to_csv(Path(processed_data_path.parent, f"{project_title}_results.csv"))


# Local test command: python src/modelling/tune/ucl_cluster_tune/VAE/VAE_tune_local.py --data_path "data/processed_data" --feature_type "cortical_thickness" --project_title "local_grid_search_test" --batch_size "128" --learning_rate "0.005" --latent_dim "10" --hidden_dim "30"
